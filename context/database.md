...
### สิ่งที่ทำ
- เขียน statements เพิ่ม 32 ข้อ (ids 29-60) ใน `src/data/mbti-statements.ts`
- รัน `npx tsx scripts/seed-mbti-statements.ts` — ลบ 28 rows เก่า + insert 60 rows ใหม่
- Distribution: 15 ต่อ dim × 4 dims = **60** (8 std + 7 rev per dim, reverse ratio 47%)
- ทุก row ยังเป็น `version=2` + `active=true`

### Schema changes
- ไม่มี — ใช้ MBTIQuestion model เดิม

### หมายเหตุ
- Quiz client ตอนนี้ใช้ `pickQuestionsForSession(pool, seed)` ดึง 24 ข้อ/session แบบ stratified deterministic
- Seed = Clerk userId (signed-in) หรือ localStorage uuid (guest) → retake → ได้ชุดเดิม comparable
- Phase 3 (schema cleanup `text→statement`) ค้างไว้ก่อน
...
## 2026-06-09 — MBTI Redesign: Link FacultyMBTIMatch to real Faculty rows

### สิ่งที่ทำ
- Audit: พบ FacultyMBTIMatch มี 64 rows แต่ `field` เป็น string label (ไม่ link Faculty จริง)
- เพิ่ม `facultyId` column (nullable) + relation ไป Faculty
- เปลี่ยน unique constraint: `[mbtiType, rank]` → `[mbtiType, facultyId]`
- Index เพิ่ม: `[mbtiType, rank]` composite + `[facultyId]`
- รัน `prisma db push --accept-data-loss` (DB เคยใช้ push ไม่ใช่ migrate dev — มี drift)
- Seed: 320 rows ใหม่ (20 per type × 16 types) ผ่าน `scripts/seed-mbti-matches.ts`
- เก็บ 64 rows เก่า (facultyId=null) ไว้เป็น fallback

### Schema changes
- `FacultyMBTIMatch`: เพิ่ม `facultyId String?`, relation `faculty Faculty?`
- `FacultyMBTIMatch`: เปลี่ยน `@@unique([mbtiType, rank])` → `@@unique([mbtiType, facultyId])`
- `FacultyMBTIMatch`: เพิ่ม `@@index([mbtiType, rank])`, `@@index([facultyId])`
- `Faculty`: เพิ่ม inverse relation `mbtiMatches FacultyMBTIMatch[]`

### Matching algorithm
- Rule-based: FacultyField → dimension affinity vector + keyword rules (14 patterns)
- Score = (dot(typeAffinity, facultyAffinity) + 4) / 8 → [0, 1]
- Top 20 per type คัดจาก score desc → minScore desc

### Verify
- INTJ → CompSci/Engineering (KMUTT, KU, CMU)
- ENFP → ศิลปกรรม + ภาษาเพื่อการสื่อสาร (BUU, UBU)
- ISFJ → พยาบาล/สาธารณสุข
- ESTP → บริหารธุรกิจ/การจัดการ

### หมายเหตุ
- ไม่กระทบ 64 row เดิม (facultyId=null ยังอยู่)
- Re-seed ได้ทุกเมื่อ: `npx tsx scripts/seed-mbti-matches.ts --top N`
- ถ้าจะ wipe: `--wipe` flag จะลบเฉพาะ row ที่มี facultyId (ไม่กระทบ fallback)
